...
public class GremlinScript extends BaseStep implements StepInterface {
	...
	public GremlinScript(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta, Trans trans) {
	...
	private void determineUsedFields(RowMetaInterface row) {
		...
		if (log.isDetailed())
			logDetailed(BaseMessages.getString(PKG, "GremlinScript.Log.UsingValuesFromInputStream", String.valueOf(data.fields_used.length)));  
	}
		...
		if (first) {
			...
			try {
				...
				try {
					...
					throw new KettleValueException(BaseMessages.getString(PKG, "GremlinScript.Log.CouldNotAttachAdditionalScripts"), e); 
				}
				...
				try {
					...
					throw new KettleValueException(BaseMessages.getString(PKG, "GremlinScript.Log.CouldNotAddDefaultConstants"), ex);
				}
				...
				try {
					...
					// es.toString());
					throw new KettleValueException(BaseMessages.getString(PKG, "GremlinScript.Log.ErrorProcessingStartScript"), es);
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		...
		if (r == null) {
			...
			try {
				...
				logError(BaseMessages.getString(PKG, "GremlinScript.Log.ErrorStackTrace") + Const.CR + Const.getStackTracker(e)); 
				setErrors(1);
				stopAll();
			}
