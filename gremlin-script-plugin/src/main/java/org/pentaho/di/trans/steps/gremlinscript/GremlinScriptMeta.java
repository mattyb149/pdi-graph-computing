...
@Step(
    id = "GremlinScript", 
    image = "gremlin-script.png",
    name = "Gremlin Script", 
    description = "A scripting step for the Gremlin-Groovy script engine", 
    categoryDescription = "Scripting")
public class GremlinScriptMeta extends BaseStepMeta implements StepMetaInterface {	
	private static Class<?> PKG = GremlinScriptMeta.class; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$

	private static final String SCRIPT_ENGINE_TAG_TYPE = "scriptEngine";
	...
	public Object clone()
	{
		GremlinScriptMeta retval = (GremlinScriptMeta)super.clone();
	...
	private void readData(Node stepnode) throws KettleXMLException{
		try	{
		  setEngineName(XMLHandler.getTagValue(stepnode, SCRIPT_ENGINE_TAG_TYPE));
		...
		{
			throw new KettleXMLException(BaseMessages.getString(PKG, "GremlinScriptMeta.Exception.UnableToLoadStepInfoFromXML"), e); 
		}
	}
	...
	public void setDefault(){
		...
		jsScripts[0] = new ScriptValuesScript(
				...
				BaseMessages.getString(PKG, "GremlinScript.Script1"),
				"//"+ BaseMessages.getString(PKG, "GremlinScript.ScriptHere") +Const.CR+Const.CR
		);
  ...
  public void getFields(RowMetaInterface row, String originStepname, 
      ...
      if (!Const.isEmpty(fieldname[i])) {
        ...
        if (replace[i]) {
          ...
          if (row.searchValueMeta(fieldname[i]) == null && Const.isEmpty(rename[i])) {
            throw new KettleStepException(BaseMessages.getString(PKG, "GremlinScriptMeta.Exception.FieldToReplaceNotFound", fieldname[i]));
          }
    ...
    retval.append("    ").append(XMLHandler.addTagValue(SCRIPT_ENGINE_TAG_TYPE, getEngineName()));
	...
	public void readRep(Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases) throws KettleException {
		try {
		  ...
		  setEngineName(rep.getStepAttributeString(id_step, SCRIPT_ENGINE_TAG_TYPE));
		...
		{
			throw new KettleException(BaseMessages.getString(PKG, "GremlinScriptMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); 
		}
	}
	...
	public void saveRep(Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step) throws KettleException {
	  try {
	    ...
	    rep.saveStepAttribute(id_transformation, id_step, SCRIPT_ENGINE_TAG_TYPE, getEngineName());
      ...
      for (int i = 0; i < fieldname.length; i++) {
        ...
        throw new KettleException(BaseMessages.getString(PKG, "GremlinScriptMeta.Exception.UnableToSaveStepInfo") + id_step, e); 
    }
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev, String input[], String output[], RowMetaInterface info, VariableSpace space, Repository repository, IMetaStore metaStore)
	{
		...
		jscx = GremlinScriptUtils.createNewScriptEngine(getEngineName());
		...
		if (prev!=null && strActiveScript.length()>0)		{
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.ConnectedStepOK",String.valueOf(prev.size())), stepMeta);  
			remarks.add(cr);
			...
			// Adding some Constants to the script
			try {
				jsscope.put("SKIP_TRANSFORMATION", Integer.valueOf(GremlinScript.SKIP_TRANSFORMATION));
				jsscope.put("ABORT_TRANSFORMATION", Integer.valueOf(GremlinScript.ABORT_TRANSFORMATION));
				jsscope.put("ERROR_TRANSFORMATION", Integer.valueOf(GremlinScript.ERROR_TRANSFORMATION));
				jsscope.put("CONTINUE_TRANSFORMATION", Integer.valueOf(GremlinScript.CONTINUE_TRANSFORMATION));
			} catch (Exception ex) {
			...
			try{
				...
				error_message="Couldn't add Input fields to GremlinScript! Error:"+Const.CR+ev.toString(); 
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
				remarks.add(cr);
			}
			...
			try{
				...
				if(strActiveStartScript != null && strActiveStartScript.length()>0){
					...
					error_message = "Found Start GremlinScript. "+ strActiveStartScriptName+" Processing OK";
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, error_message, stepMeta); 
					remarks.add(cr);
				}
			}catch(Exception e){
				error_message="Couldn't process Start GremlinScript! Error:"+Const.CR+e.toString(); 
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
				remarks.add(cr);				
			}
			...
			try{
				...
				//cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.ScriptCompiledOK"), stepinfo); 
				//remarks.add(cr);
				...
				try{
					...
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.ScriptCompiledOK2"), stepMeta); 
					remarks.add(cr);
					
					if (fieldname.length>0){
						StringBuffer message = new StringBuffer(BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.FailedToGetValues",String.valueOf(fieldname.length))+Const.CR+Const.CR);  
					...
					error_message=BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.CouldNotExecuteScript")+Const.CR+jse.toString(); 
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
					remarks.add(cr);
				}catch(Exception e){
					//Context.exit(); TODO AKRETION NOT SURE
					error_message=BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.CouldNotExecuteScript2")+Const.CR+e.toString(); 
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
					remarks.add(cr);
				}
				
				// Checking End GremlinScript
				try{
					if(strActiveEndScript != null && strActiveEndScript.length()>0){
						/* Object endScript = */ jscx.eval(strActiveEndScript, jsscope);
						error_message = "Found End GremlinScript. "+ strActiveEndScriptName+" Processing OK";
						cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, error_message, stepMeta); 
						remarks.add(cr);
					}
				}catch(Exception e){
					error_message="Couldn't process End GremlinScript! Error:"+Const.CR+e.toString(); 
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
					remarks.add(cr);				
				}
			}catch(Exception e){
				//Context.exit(); TODO AKRETION NOT SURE
				error_message = BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.CouldNotCompileScript")+Const.CR+e.toString(); 
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
				remarks.add(cr);
			}
		}else{
			//Context.exit(); TODO AKRETION NOT SURE
			error_message = BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.CouldNotGetFieldsFromPreviousStep"); 
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, error_message, stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length>0){
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.ConnectedStepOK2"), stepMeta); 
			remarks.add(cr);
		}else{
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.NoInputReceived"), stepMeta); 
			remarks.add(cr);
		}
	}
	...
	public boolean getValue(Bindings scope, int i, Value res, StringBuffer message)
	{
		...
		if (fieldname[i]!=null && fieldname[i].length()>0)
		{
			...
			{
				message.append(BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.ErrorRetrievingValue",fieldname[i])+" : "+e.toString());  
				error_found=true;
			}
			res.setLength(length[i], precision[i]);
				
			message.append(BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.RetrievedValue",fieldname[i],res.toStringMeta()));  
		}
		else
		{
			message.append(BaseMessages.getString(PKG, "GremlinScriptMeta.CheckResult.ValueIsEmpty",String.valueOf(i)));  
			error_found=true;
		}
	...
	{
		return new GremlinScript(stepMeta, stepDataInterface, cnr, transMeta, trans);
	}

	public StepDataInterface getStepData()
	{
		return new GremlinScriptData();
	}
	...
	public void parseXmlForAdditionalClasses() throws KettleException{
		try {
			...
			throw new KettleException(BaseMessages.getString(PKG, "GremlinScriptMeta.Exception.UnableToParseXMLforAdditionalClasses"), e); 
		}
	}
	...
	private static Class<?> LoadAdditionalClass(String strJar, String strClassName) throws KettleException{
		try{
			...
			throw new KettleException(BaseMessages.getString(PKG, "GremlinScriptMeta.Exception.UnableToLoadAdditionalClass"), e); 
		}
	}
